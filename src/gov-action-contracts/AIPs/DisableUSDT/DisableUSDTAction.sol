...
contract DisableUSDTAction {
    ...
    IL1GatewayRouter public constant gatewayRouter =
    ...
    function perform() external {
        ...
        gatewayRouter.setGateways{value: address(this).balance}(
            tokens, gateways, 0, 0, address(this).balance
        );

        // check that the gateway was set correctly
        require(
            gatewayRouter.getGateway(usdt) == address(0), "DisableUSDTAction: gateway not disabled"
        );
    }
}
